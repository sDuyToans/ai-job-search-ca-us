...
- **`/dashboard`** starts `tools/dashboard_server.py`, a stdlib-only local web server serving the same stats/charts/table as `/html-report` but recomputed live on every page load (no dependencies, no re-run needed after `/outcome`), plus click-through links to each application's compiled CV/cover letter PDF. Binds to `127.0.0.1` only. `/dashboard stop` shuts it down.
