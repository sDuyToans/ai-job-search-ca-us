...
1. **`cv_draft.tex` and `cover_letter.tex`** - copy (never move) the submitted files. Locate them via the tracker row's `cv_file`/`cover_letter_file` columns; if those are empty, the date is not known in advance, so glob `applications/*/<company>_<role>/cv.tex` and `applications/*/<company>_<role>/cover_letter.tex` (if more than one date folder matches the slug, ask the user which one). If a file already exists in the archive, leave it - the archived version is what was actually submitted. If no draft files exist (application made outside `/apply`), skip with a note.
