...
- Read any existing `applications/*/*/cv<CV_EXT>` file as a structural reference
- Read any existing `applications/*/*/cover_letter<COVER_EXT>` file as a structural reference
- If no application folder exists yet, fall back to `cv/main_example.tex` and `cover_letters/cover_example.tex`
...
**Create the application folder before writing anything:** `applications/<YYYY-MM-DD>/<company>_<role>/`, where `<YYYY-MM-DD>` is today's actual date (the date this application is being drafted — never a placeholder), and `<company>_<role>` uses the same lowercase, underscore-joined slug convention as existing folders. Then set up the cover letter's shared assets once:

```bash
mkdir -p applications/<YYYY-MM-DD>/<company>_<role>
...
(The symlinks go up three levels — company folder → date folder → `applications/` → repo root — one more than before the date folder was added.)

(Skip the symlinks if a custom template is active and does not use `cover.cls`.)
...
### CV (`applications/<YYYY-MM-DD>/<company>_<role>/cv<CV_EXT>`)
...
### Cover Letter (`applications/<YYYY-MM-DD>/<company>_<role>/cover_letter<COVER_EXT>`)
...
<CV_DRAFT file="applications/<COMPANY>_<ROLE>/cv<CV_EXT>">
<INSERT_CV_DRAFT_HERE>
</CV_DRAFT>

<COVER_LETTER_DRAFT file="applications/<COMPANY>_<ROLE>/cover_letter<COVER_EXT>">
...
```json
{
  "file": "applications/<COMPANY>_<ROLE>/cv<CV_EXT>" | "applications/<COMPANY>_<ROLE>/cover_letter<COVER_EXT>",
...
Both compile from the same `applications/<YYYY-MM-DD>/<company>_<role>/` directory — no `cd` between them. The cover letter's `cover.cls`/`OpenFonts` symlinks (set up in Step 2) must already be present in that folder.
...
**CV (`applications/<YYYY-MM-DD>/<company>_<role>/cv.pdf`):**
...
**Cover letter (`applications/<YYYY-MM-DD>/<company>_<role>/cover_letter.pdf`):**
- [ ] Exactly 1 page
...
- `applications/<YYYY-MM-DD>/<company>_<role>/cv<CV_EXT>`
- `applications/<YYYY-MM-DD>/<company>_<role>/cover_letter<COVER_EXT>`
