...
2. **Fallbacks** (the application may predate `/outcome`): posting via WebFetch on the tracker row's `source` URL, or ask the user to paste it; CV via `applications/<company>_<role>/cv.tex` and cover letter via `applications/<company>_<role>/cover_letter.tex`. State plainly which context is missing rather than guessing - and suggest `/outcome <company>` to build the archive for next time.
