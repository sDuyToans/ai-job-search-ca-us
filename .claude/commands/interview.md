...
2. **Fallbacks** (the application may predate `/outcome`): posting via WebFetch on the tracker row's `source` URL, or ask the user to paste it; CV and cover letter via the tracker row's `cv_file`/`cover_letter_file` columns, or if those are empty, glob `applications/*/<company>_<role>/cv.tex` and `applications/*/<company>_<role>/cover_letter.tex` (the date is not known in advance; ask the user if more than one date folder matches). State plainly which context is missing rather than guessing - and suggest `/outcome <company>` to build the archive for next time.
