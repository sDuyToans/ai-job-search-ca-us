...
- "health" to run the portal health check only (Step 4.75), without searching, deduplicating, or presenting jobs - e.g. "/scrape health", or "/scrape health eluta-search" to probe one portal even if disabled
...
```json
{
  "seen": {
    "<url_or_company_title_key>": {
      ...
      "portal": "<source portal skill, e.g. eluta-search>"
    }
  }
}
```
